...
def test_scalar_vec_real_known_values():
    ...
    computed_output_dumb  = temp.scalar_vec_real(scalar, vector)
...
def test_outer_real_scalar_multiplication():
    ...
    a_outer_cb = []
    outer_real = {
        'simple' : temp.outer_real_simple,
        'row' : temp.outer_real_row,
        'column' : temp.outer_real_column
    }
...
def test_outer_complex_compare_numpy_outer():
    ...
    output = []
    for function in ['simple', 'row', 'column']:
        output.append(temp.outer_complex(input1, input2, function))
...
def test_outer_complex_invalid_function():
    ...
    for invalid_function in ['Vasco', 'xxxxx', 'rows']:
        with pytest.raises(ValueError):
            temp.outer_complex(np.ones(3), np.ones(3), function=invalid_function)
